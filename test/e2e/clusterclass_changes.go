...
func modifyControlPlaneViaClusterClassAndWait(ctx context.Context, input modifyClusterClassControlPlaneAndWaitInput) {
	...
	Eventually(func(g Gomega) error {
		...
		replicas, ok, err := unstructured.NestedFieldCopy(controlPlane.Object, contract.ControlPlane().Replicas().Path()...)
		g.Expect(err).ToNot(HaveOccurred())
		g.Expect(ok).To(BeTrue())

		readyReplicas, ok, err := unstructured.NestedFieldCopy(controlPlane.Object, contract.ControlPlane().ReadyReplicas().Path()...)
		g.Expect(err).ToNot(HaveOccurred())
		g.Expect(ok).To(BeTrue())

		updatedReplicas, ok, err := unstructured.NestedFieldCopy(controlPlane.Object, contract.ControlPlane().UpdatedReplicas().Path()...)
		g.Expect(err).ToNot(HaveOccurred())
		g.Expect(ok).To(BeTrue())

		g.Expect(replicas).To(BeComparableTo(readyReplicas), "Machine deployment does not have expected replicas")
		g.Expect(replicas).To(BeComparableTo(updatedReplicas), "Machine deployment does not have expected updated replicas")
...
func rebaseClusterClassAndWait(ctx context.Context, input rebaseClusterClassAndWaitInput) *clusterv1.ClusterClass {
	...
	if input.ControlPlaneChanged {
		Eventually(func(g Gomega) {
			...
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(afterControlPlane.GetGeneration()).ToNot(Equal(beforeControlPlane.GetGeneration()),
				"ControlPlane generation should be incremented during the rebase because ControlPlane expected to be changed.")

			replicas, ok, err := unstructured.NestedFieldCopy(afterControlPlane.Object, contract.ControlPlane().Replicas().Path()...)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(ok).To(BeTrue())

			readyReplicas, ok, err := unstructured.NestedFieldCopy(afterControlPlane.Object, contract.ControlPlane().ReadyReplicas().Path()...)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(ok).To(BeTrue())

			updatedReplicas, ok, err := unstructured.NestedFieldCopy(afterControlPlane.Object, contract.ControlPlane().UpdatedReplicas().Path()...)
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(ok).To(BeTrue())
